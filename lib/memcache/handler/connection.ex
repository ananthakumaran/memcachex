defmodule Memcache.Handler.Connection do
  @moduledoc """
  ...
  * `:coder` - (module | {module, options}) Can be either a module or
  ...
  def get(server, key, opts \\ []) do
    GenServer.call(server, {:execute_k, :GET, [key], opts})
  end
  ...
  def connection_pid(server) do
    GenServer.call(server, :connection_pid)
  end
  ...
  defp decode_response({:ok, value}, state) when is_binary(value) do
    {:ok, decode(state, value)}
  end
  defp decode_response({:ok, value, cas}, state) when is_binary(value) do
  ...
  defp key_with_namespace(state, key) do
  ...
  def handle_call(:connection_pid, _from, state) do
    {:reply, Map.get(state, :connection), state}
  end
